from appwrite.client import Client
from appwrite.services.databases import Databases
import hashlib
import datetime
import json
import asyncio

# Initialize the Appwrite client
client = Client()
client.set_endpoint('https://cloud.appwrite.io/v1') \
      .set_project('666927680026e4e78388') \
      .set_key('')

database = Databases(client)

async def shorten_url(original_url, custom_alias=None, expiration_time=None, user_id=None):
    url_id = custom_alias if custom_alias else hashlib.md5(original_url.encode()).hexdigest()[:6]
    short_url = f"https://short.url/{url_id}"
    
    data = {
        'original_url': original_url,
        'short_url': short_url,
        'custom_alias': custom_alias,
        'created_at': datetime.datetime.now().isoformat(),
        'expiration_date': expiration_time,
        'user_id': user_id
    }

    try:
        result = await database.create_document(collection_id='urls', document_id='unique()', data=data)
        return result
    except Exception as e:
        print(f"Error creating document: {e}")
        return None

async def main(context):
    payload = json.loads(context['data'])
    original_url = payload.get('originalURL')
    user_id = payload.get('user_id')
    custom_alias = payload.get('custom_alias')
    expiration_time = payload.get('expiration_time')

    result = await shorten_url(original_url, custom_alias, expiration_time, user_id)
    print(result)

# Example usage with context
context = {
    'data': json.dumps({
        'originalURL': 'https://example.com',
        'user_id': 'user123',
        'custom_alias': 'example',
        'expiration_time': (datetime.datetime.now() + datetime.timedelta(days=7)).isoformat()
    })
}

# Run the main function
asyncio.run(main(context))
